...
```{r richness-rt, fig.height = 5, fig.width = 8, fig.cap = "Sample richness against residence times across years. The observations nearest to zero hours were all from the South Bay meadow. "}
mod_df <- specs_ran2 %>% 
...
ggplot(mod_df) +
  geom_point(aes(y = richness, x = rt2)) +
  facet_wrap(~year) +
  labs(x = "Residence time (hours)",
       y = "Sample richness") +
  theme_bw()
```

```{r depth-rt, fig.height = 5, fig.width = 8, fig.cap = "Sample richness against water depth (m NAVD 88) across years."}
ggplot(mod_df) +
  geom_point(aes(y = richness, x = d2, color = insideSeagrass)) +
  facet_wrap(~year) +
  labs(x = "Depth (m NAVD 88)",
       y = "Sample richness") +
  theme_bw()
```